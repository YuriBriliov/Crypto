      ...
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div
          class="bg-white overflow-hidden shadow rounded-lg border-purple-800 border-solid cursor-pointer"
           v-for="(item, index) in tickers" :key="index"
           @click="select(item)"
           :class="{
             'border-4' : sel === item
           }"
...
export default {
  ...
  methods: {
    add(){
      ...
      setInterval(async()=>{
        const inter = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=da445d52821d5dd9c7552c9e82add1761f8183f6bf482d2172374456811619a0`)
        const data = await inter.json()
        this.tickers.forEach((item) => {
          if(item.name === currentTicker.name){
